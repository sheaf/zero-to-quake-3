...
import GHC.Base hiding(foldr, (:|), sequence)
...
repeatV a = case (Proxy @1) %<=? (Proxy @n) of
                 LE Refl   -> a :. (repeatV a :: V (n-1) a)
...
instance (KnownNat n, Storable a) => Storable (V n a) where
  ...
  peek ptr = sequence . fmap (peekElemOff (castPtr ptr)) $ ixVec
      where ixVec :: V n Int
            ixVec = unfoldr pred (fromInteger $ natVal (Proxy @n))
...
unfoldr :: forall n a. KnownNat n => (a -> a) -> a -> V n a
unfoldr f a = case (Proxy @1) %<=? (Proxy @n) of
                   LE Refl   -> let b = f a in b :. (unfoldr f b :: V (n-1) a)
                   NLE nle _ -> case deduceZero nle of
                                     Refl -> (:|)
...
class (Num (Scalar v), RealFloat (Scalar v), Show (Scalar v), KnownNat (Dim v)) => Vector v where
  type Scalar v :: Type
  type Dim v :: Nat
  (^+^), (^-^) :: v -> v -> v
  (*^)  :: Scalar v -> v -> v
...
instance (Num a, RealFloat a, Show a, KnownNat n) => Vector (V n a) where
  type Scalar (V n a) = a
  type Dim (V n a) = n
  (^+^) = zipWithV (+)
  (^-^) = zipWithV (-)
...
instance (Num a, RealFloat a, Show a, KnownNat n) => Inner (V n a) where
  (^.^) = (foldr (+) 0 .) . zipWithV (*)
...
instance (Num a, RealFloat a, Show a) => Cross (V 3 a) where
  (x :. y :. z :. (:|)) ^×^ (x' :. y' :. z' :. (:|)) = a :. b :. c :. (:|)
    where a = y * z' - z * y'
          b = z * x' - x * z'
          c = x * y' - y * x'
...
transpose rows = repeatV (:|) `addCols` rows 
...
fromColumns = addCols ( repeatV (:|) :: M m 0 a)
...
lookAt :: (Show a, Num a, RealFloat a) => V 3 a -> V 3 a -> V 3 a -> M 4 4 a
lookAt eye centre up =  (        xa <++> (-xd :. (:|))) 
                     :. (        ya <++> (-yd :. (:|)))
                     :. ((-1) *^ za <++> ( zd :. (:|)))
                     .: (0 :. 0 :. 0 .: 1)
...
perspective :: (Show a, Num a, RealFloat a) => a -> a -> a -> a -> M 4 4 a
